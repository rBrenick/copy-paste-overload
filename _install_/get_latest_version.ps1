...
$TOOL_GIT_REPO = "https://github.com/rBrenick/copy-paste-overload/archive/master.zip" # modify these after uploading the tool
$UPDATE_PATHS = @("docs", "copy_paste_overload", "README.md", "setup.py")

$CURRENT_TOOL_FOLDER = (Get-Item -Path ".").FullName
$CURRENT_copy_paste_overload = (Get-Item -Path ".").Name

$NEW_VERSION_ZIP = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_copy_paste_overload + "_update.zip"
$NEW_VERSION_ZIP_FOLDER = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_copy_paste_overload + "_update"
...
foreach ($element in $UPDATE_PATHS) {
    ...
    $src_file_path = "$NEW_VERSION_ZIP_FOLDER\$CURRENT_copy_paste_overload-master\$element"
